...
class FeedVC: UIViewController, UITableViewDelegate, UITableViewDataSource, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any]) {
        if let image = info[UIImagePickerControllerEditedImage] as? UIImage {
            ...
            print("GUHA: A valid image wasn't selected")
        }
    ...
    @IBAction func postBtnTapped(_ sender: AnyObject) {
        guard let caption = captionField.text, caption != "" else {
            print("GUHA: Caption must be entered")
            return
        }
        guard let img = imageAdd.image, imageSelected == true else {
            print("GUHA: An image must be selected")
            return
        }
        ...
        if let imgData = UIImageJPEGRepresentation(img, 0.2) {
            ...
            DataService.ds.REF_POST_IMAGES.child(imgUid).put(imgData, metadata: metadata) { (metadata, error) in
                if error != nil {
                    print("GUHA: Unable to upload image to Firebasee torage")
                } else {
                    print("GUHA: Successfully uploaded image to Firebase storage")
